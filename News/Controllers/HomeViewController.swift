...
class HomeViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, IndicatorInfoProvider {
    ...
    func fetchData() {
        ...
        Services.fetchEverything { result in
            switch result {
    ...
    func fetchMoreData() {
        self.activityView.startAnimating()
        Services.fetchEverything(parameters: ["page": String(self.pageNumber)]) { result in
            switch result {
            case .success(let articles):
                self.articles += articles.articles
                DispatchQueue.main.async {
